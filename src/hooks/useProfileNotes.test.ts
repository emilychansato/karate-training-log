...
vi.mock('../lib/supabaseClient', () => {
  const maybeSingle = vi.fn().mockResolvedValue({
    data: {
      why_i_train: 'For my future self.',
      belt_rank: 'Green',
      club_name: 'North Shore Karate',
      primary_discipline: 'kumite',
    },
    error: null,
  })
...
describe('useProfileNotes', () => {
  ...
  it('saveProfileInfo upserts belt rank, club, and primary discipline scoped to the signed-in user', async () => {
    const { result } = renderHook(() => useProfileNotes())
    await waitFor(() => expect(result.current.loading).toBe(false))

    let response: { error: string | null } = { error: 'unset' }
    await act(async () => {
      response = await result.current.saveProfileInfo({
        beltRank: 'Brown',
        clubName: 'North Shore Karate',
